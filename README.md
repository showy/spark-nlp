...
To use the most recent version just add the `--packages JohnSnowLabs:spark-nlp:1.4.1` to you spark command
...
```sh
spark-submit --packages JohnSnowLabs:spark-nlp:1.4.1
```
...
#### SBT
```sbtshell
libraryDependencies += "com.johnsnowlabs.nlp" % "spark-nlp_2.11" % "1.4.1"
```
